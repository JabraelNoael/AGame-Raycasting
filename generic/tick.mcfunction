...
execute as @a[scores={raycast=0..}] if block ~ ~ ~ #parent:raycast unless entity @e[nbt=!{Invulnerable:1b},type=!#parent:raycast,type=!player,dz=0,sort=nearest,limit=1] positioned ^ ^ ^.5 run function raycast:generic/tick
